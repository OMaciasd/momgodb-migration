...
try:
    ...
    category_pipeline_mongo = [
        {"$lookup": {
         "from": "product_categories", "localField": "category_id",
    ...
    warehouse_pipeline_mongo = [
        {"$unwind": "$items"},
        {"$lookup": {
         "from": "inventories", "localField": "items.product_id",
         "foreignField": "product_id", "as": "inventory"
         }
         },
        {"$unwind": "$inventory"},
        {"$lookup": {
         "from": "warehouse", "localField": "inventory.warehouse_id",
         "foreignField": "_id", "as": "warehouse"
         }
         },
        {"$unwind": "$warehouse"},
    ...
    generate_graph(
        pg_results_category, "Productos por Categoría (PostgreSQL)",
        "pg_category_chart.png"
    )
    generate_graph(
        mongo_results_category, "Productos por Categoría (MongoDB)",
        "mongo_category_chart.png"
    )
